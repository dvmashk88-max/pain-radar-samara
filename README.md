...
Telegram-бот-антикоуч, который жёстко разбирает бизнес-идеи и превращает разбор в короткое шоу.

Пользователь присылает идею одним сообщением, а бот прогоняет её через трёх персонажей:

- 💀 Разъёбщик — ищет самообман и слабые допущения
- ⚖️ Прокурор — бьёт по рынку, деньгам, конкуренции и рискам
- 😈 Адвокат — ищет единственный сценарий, при котором идея может выжить

В конце бот даёт тест на выживание, вердикт и продолжает разговор наводящими вопросами:
...
Любое обычное текстовое сообщение бот воспринимает как идею для разбора. После первого ответа бот держит контекст разговора в памяти процесса и проводит 2–3 раунда: клиент, деньги, канал продаж, финальный итог.

Если пользователь приносит новую идею внутри старого разговора, бот сохраняет её как новую и начинает новый разбор. Архив идей доступен только админу.
...
ai_analyzer.py  — разбор идей и продолжение диалога через OpenRouter
scanner.py      — старый сканер, сохранён в репозитории, но не используется в UI
database.py     — SQLite: старая база сигналов плюс новые таблицы идей и статистики
